...
const SvgContainer: React.FC<Props> = ({
    ...
    const init = () => {
        ...
        const chartAreaClip = svgSelector
            .select('g')
            .append('clipPath')
        ...
        const xDomain = [timeExtent[0], timeExtent[timeExtent.length - 1]];
    ...
    const onZoomed = () => {
        ...
        const newRange = [0, width].map(
            (d) => d3Event.transform.applyX(d) as number
        );

        scales.x.range(newRange);

        scales.xScaleBand.range([newRange[0], newRange[1]]);
    ...
    const resizeHandler = () => {
        ...
        select(`#${ChartClipPathId}`).select('rect').attr('width', newWidth);
