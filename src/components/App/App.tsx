...
const App: React.FC<Props> = () => {
    ...
    const [activeLayer, setActiveLayer] =
        React.useState<GldasLayerName>('Soil Moisture');

    const [selectedTimeExtentItem, setSelectedTimeExtentItem] =
        React.useState<TimeExtentItem>();

    const [previewTimeExtentItem, setPreviewTimeExtentItem] =
        React.useState<TimeExtentItem>();

    const [queryLocation, setQueryLocation] = React.useState<IPoint>();

    const { gldasDataResponse, isLoading, isFailed, resetIsFailed } =
    ...
    const getBottomPanel = () => {
        ...
        if (isLoading) {
            return (
                <BottomPanel isLoading={isLoading} isMobile={isMobile}>
                    <></>
                </BottomPanel>
            );
        }
            ...
            return (
                <BottomPanel isLoading={isLoading} isMobile={isMobile}>
                    <ChangeInStorageIndicator
                        ...
                        timeExtentItem={
                            previewTimeExtentItem || selectedTimeExtentItem
                        }
                    />
    ...
    return selectedTimeExtentItem ? (
        <>
            <TopNav infoIconOnClick={setIsInfoModalOpen.bind(this, true)} />

            <MapView
                paddingBottom={
                    shouldShowBottomPanel ? UIConfig['bottom-panel-height'] : 0
                }
            ...
            >
                ...
                <QueryLocationGraphic geometry={queryLocation} />

                <SearchWidget onSelect={setQueryLocation} />
            ...
            <MobileDeviceAlert isVisible={isMobile} />

            <ErrorAlert isVisible={isFailed} onClose={resetIsFailed} />
        </>
    ) : null;
};
