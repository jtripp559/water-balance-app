...
const SummaryCard: React.FC<Props> = ({
        ...
        return (
            <div className="font-size--3">
                <SummaryTableRow>
                    ...
                    >
                        {data.Precipitation[index].value} mm
                    </span>
                </SummaryTableRow>
                ...
                <SummaryTableRow>
                    ...
                    >
                        {data.Evapotranspiration[index].value} mm
                    </span>
                </SummaryTableRow>
                ...
                <SummaryTableRow>
                    ...
                    >
                        {data['Soil Moisture'][index].value} mm
                    </span>
                </SummaryTableRow>
    ...
    const getDescription = () => {
                ...
                style={{
                    color:
                        changeInStorageVal >= 0
                            ...
                            : UIConfig['water-flux-line-color'],
                }}
            >
        ...
        const soilMoisture4SelectedMonth = gldasDataByMonth['Soil Moisture'][
            monthIndex
        ].map((d) => d.value);

        const avgSoilMoisture4SelectedMonth = average(
            soilMoisture4SelectedMonth
        );

        const pctDiffSoilMoistureFromAve =
            ((soilMoistureValue - avgSoilMoisture4SelectedMonth) /
                avgSoilMoisture4SelectedMonth) *
            100;
        ...
        const compare2Avg =
            pctDiffRounded === 0
                ? 'about the average'
                : `${pctDiffRounded}% ${
                      pctDiffSoilMoistureFromAve >= 0 ? 'above' : 'below'
                  } average`;
        ...
        return (
            <p className="font-size--3 leader-half trailer-half">
                {changeInStorageText} of water was{' '}
                {changeInStorageVal >= 0 ? 'recharged into' : 'depleted from'}{' '}
                storage this month. Total soil moisture is {compare2AvgText} for{' '}
                {format(date, 'MMMM')}.
            </p>
        );
    };
