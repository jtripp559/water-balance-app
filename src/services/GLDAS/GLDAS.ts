...
const LayersInfo =
    location.host !== 'livingatlas.arcgis.com'
        ? GldasLayersInfoDEV
        : GldasLayersInfo;
...
export const getGLDASdata = async (
    queryLocation: IPoint
): Promise<{
            ...
            .then((responses) => {
                ...
                for (let i = 0, len = responses.length; i < len; i++) {
                    ...
                    identifyResults[layerName] =
                        processGldasResult(originalValues);
                }

                const identifyResultsByMonth =
                    groupGldasDataByMonth(identifyResults);
